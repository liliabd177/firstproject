...
import streamlit
from streamlit import slider, number_input, sidebar, text_input, selectbox
import pandas as pd
...
# average_reating = books_df['User Reating'].mean()
# average_reating = books_df['User Price'].mean()
#
# col1,col2,col3,col4 = st.columns(4)
# col1.metric("Toal books", total_books)
# col2.metric("Unique titles", unique-titles)
...
#
# st.subheader("genre distribuition")
...
# sidebar.header("add new book")
# with sidebar.form("book_form"):
#     new_name = text_input("author")
#     new_user_rating = slider("user rating", 0.0, 5.0, 0, 1)
#     new_reviews = number_input("reviews ", min_value =0, step=1)
#     new_price = number_input("price", min_value=0, step=1)
#     new_year = number_input("year", min_value=2009, max_value=2022, step=1)
#     new_genre = selectbox("genre", books_df['genre'].unique())
#     submit_button = streamlit.form_submit_button(label="add book")
# 
# 
#     if submit_button:
#     new_data = {
...
# 
# 
#         filtered_books_df = filtered_books_df[(fitered_books_df['user rating'] >= min_rating) & (filtered_books_df['price'] <= max_price.)]
